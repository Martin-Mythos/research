...
3. **Boundary:** inject a first-attempt exception in one worker and a first-attempt timeout in another. Use two attempts, aggregate all outcomes, and inspect state/events. Important control: the timeout/retry policy belongs to this harness, not QM's `TaskStore`.
