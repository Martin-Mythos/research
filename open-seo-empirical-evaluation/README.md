...
```bash
# 0. 从本研究仓库根目录开始，并保存其绝对路径
RESEARCH_REPO="$(pwd)"

# 1. 克隆并固定到本研究实际评估的 OpenSEO revision
OPEN_SEO_REVISION=e9955b46cb9311c02f905337ec580995eaba65de
git clone --no-checkout https://github.com/every-app/open-seo /tmp/open-seo-target
cd /tmp/open-seo-target
git fetch --depth 1 origin "$OPEN_SEO_REVISION"
git checkout --detach "$OPEN_SEO_REVISION"
test "$(git rev-parse HEAD)" = "$OPEN_SEO_REVISION"
...
```bash
python3 "$RESEARCH_REPO/open-seo-empirical-evaluation/scripts/cost_scenario.py"
```
