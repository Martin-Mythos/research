...
| Cost Optimization Efficacy | 2 | mock harness 在人为价格假设下产生 68.26% 模拟差异 | 自动测试验证了 baseline 的同 token workload 算术，但没有真实 token/cost telemetry，不能据此证明 Shifu 的实际节省。 |
...
修订版 harness 的 5 项测试确实验证了研究协议自身的路由顺序、context lineage、错误修复边界与成本算术；这些测试不会提升 Shifu 的 Routing 或 Cost 分数，因为被测控制流由研究者编写，而非来自目标仓库。
