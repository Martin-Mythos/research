...
方法：编写本地 mock harness，模拟三个 agent 的输入/输出 JSON，包括 `task_id`、`model`、`input_context_hash`、`output_context_hash`、`schema_version`。检查每个阶段是否保留 app 名称、功能列表、验收标准。
...
方法：mock harness 先生成一个带语法错误的 Python CLI 文件，然后运行 `python -m py_compile`。失败后把错误报告路由给 Lula/Mini 生成修复 spec，再由 Spark 输出修复版。
